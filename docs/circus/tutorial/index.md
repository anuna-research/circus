...
```sh
cat /tmp/ring/.git/circus/greeter/1/record.json | grep -o '"sentinel_path":[^,]*'
```

The path is `/tmp/ring/.git/circus/greeter/1/sentinel`. It does not exist yet.
The worker creates it, once, when the work is done.
...
```sh
S=/tmp/ring/.git/circus/greeter/1/sentinel
...
The command blocks while the agent runs. In another terminal, watch it:

```sh
tmux attach -t circus-greeter-1
```
